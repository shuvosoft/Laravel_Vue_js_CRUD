           ...
           <table class="table table-hover table-bordered table-striped">
             ...
             <tr v-show="customers.length" v-for="(customer,index) in customers" :key="customer.id">
               ...
               <td class="text-center">
                 <button type="button"  class="btn btn-info btn-sm" @click="show(customer)">
     ...
     <div class="modal-dialog" role="document">
       <div class="modal-content">
         <div class="modal-header">
           <h5 class="modal-title" id="customerModalLabel">{{editMode ? 'Edit' : 'Create'}} Customer</h5>
           <button type="button" class="close" data-dismiss="modal" aria-label="Close">
             <span aria-hidden="true">&times;</span>
           </button>
         </div>

<!--         <form @submit.prevent="store" @keydown="form.onKeydown($event)">--> <!-- only store -->
         <form @submit.prevent="editMode ? update() : store()" @keydown="form.onKeydown($event)"> <!-- store and update  same form -->
           <div class="modal-body">
             ...
             <div class="form-group">
               ...
               <input v-model="form.email" type="email"
                   name="email" class="form-control" :class="{ 'is-invalid': form.errors.has('email') }">
               <has-error :form="form" field="email"></has-error>
             </div>
             <div class="form-group">
               <label>Phone</label>
               <input v-model="form.phone"
                   type="tel" name="phone" class="form-control" :class="{ 'is-invalid': form.errors.has('phone') }">
    ...
    export default {
      ...
      methods:{
        ...
        searchData(){
          this.$Progress.start()
          axios.get(
              "/api/search/customers/" +this.queryFiled +"/" + this.query + "?page=" +this.pagination.current_page
          )
          .then(response =>{
            // console.log(response);
            this.customers = response.data.data;
        ...
        create(){
          this.editMode = false
          this.form.reset() // after submit data then reload modal
          this.form.clear() // if have any error then clear this
